...
export default function WorkoutPage() {
  ...
  useEffect(() => {
    async function fetchUserAndWorkout() {
      try {
        ...
        // ✅ Determine Goal (prefer mainGoal, fallback to weightGoal)
        const goalText =
          (userData.user.mainGoal || userData.user.weightGoal || "").toLowerCase() || "";
        const userGoal = goalText.includes("gain")
          ? "gain"
        ...
        // ✅ Determine Level
        // Prefer registered activity, else fallback to localStorage or default
        const userLevel =
          userData.user.activity?.toLowerCase() ||
          storedUser?.activity?.toLowerCase() ||
          "beginner";

        // ✅ Fetch plan from MongoDB via API
  ...
  const progress =
    ...
    // Count only completed days that belong to the active workout days
